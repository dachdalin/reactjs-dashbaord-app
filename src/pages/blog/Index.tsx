    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-white/60 backdrop-blur-sm">
      <div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl bg-white border border-slate-200 shadow-2xl">
        <div className="sticky top-0 flex items-center justify-between px-6 py-4 border-b border-slate-200 bg-white">
          <h2 className="text-xl font-bold text-slate-950">{post ? "Edit Post" : "New Post"}</h2>
        ...
        <form onSubmit={handleSave} className="p-6 space-y-5">
          {error && (
            <div className="p-3 rounded-xl bg-slate-50 border border-slate-200 text-slate-950 text-sm">{error}</div>
          )}
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-3 rounded-xl bg-white border border-slate-200 text-slate-950 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-sky-400/40 focus:border-transparent"
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-4 py-3 rounded-xl bg-white border border-slate-200 text-slate-950 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-sky-400/40 focus:border-transparent resize-none"
            ...
            <div className="flex items-end pb-1">
              <label className="flex items-center gap-3 cursor-pointer">
                <div
                  ...
                  className={`w-12 h-6 rounded-full relative transition-colors ${status ? "bg-sky-500 text-slate-950" : "bg-slate-100"} cursor-pointer`}
                >
                  <div className={`absolute top-1 w-4 h-4 bg-sky-500 rounded-full transition-all ${status ? "right-1" : "left-1"}`} />
                </div>
          ...
          <div>
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-3 py-2 rounded-lg bg-white border border-slate-200 text-slate-950 text-sm placeholder-slate-400 focus:outline-none focus:ring-1 focus:ring-sky-400/40"
                placeholder="Create new tag..."
              />
              <button type="button" onClick={handleCreateTag} className="px-3 py-2 rounded-lg bg-slate-50 text-slate-700 hover:bg-slate-100 text-sm transition-colors">
                Add
              </button>
            </div>
          </div>
          ...
          <div className="flex justify-end gap-3 pt-2">
            <button type="button" onClick={onClose} className="px-5 py-2.5 rounded-xl border border-slate-200 text-slate-700 hover:bg-slate-50 transition-colors">
              Cancel
            </button>
            <button
              type="submit" disabled={saving}
              className="px-5 py-2.5 rounded-xl bg-sky-500 text-slate-950 font-medium hover:opacity-90 transition-opacity disabled:opacity-50"
            >
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
        ...
        {isAuthor() && (
          <button
            ...
            className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-sky-500 text-slate-950 text-sm font-medium shadow-lg shadow-sky-200/70 hover:shadow-slate-200/70 transition-all hover:scale-105"
          >
      ...
      {actionError && (
        <div className="p-3 rounded-xl bg-slate-50 border border-slate-200 text-slate-950 text-sm flex items-center gap-2">
          ...
          {actionError}
          <button onClick={() => setActionError(null)} className="ml-auto text-slate-500 hover:text-sky-700">✕</button>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map((post) => (
            <article
              key={post.id}
              className="group rounded-2xl overflow-hidden bg-white backdrop-blur-xl border border-slate-200 hover:border-slate-200 transition-all hover:-translate-y-1 hover:shadow-2xl hover:shadow-slate-200/70"
            >
              {/* Image / Type banner */}
              {post.image ? (
                <div className="relative h-48 overflow-hidden">
                  ...
                  <span className="absolute bottom-3 left-3 px-3 py-1 rounded-full text-xs font-semibold bg-sky-500 text-slate-950">
                    {post.type}
                  </span>
                </div>
              ) : (
                <div className="h-14 bg-sky-500 text-slate-950 flex items-center px-4">
                  <span className="text-slate-950 font-semibold text-sm tracking-wide">{post.type}</span>
                </div>
              )}
              ...
              <div className="p-5">
                <div className="flex items-start justify-between gap-2 mb-2">
                  <h3 className="text-lg font-semibold text-slate-950 line-clamp-2 group-hover:text-sky-700 transition-colors flex-1">
                    {post.title}
                  </h3>
                  {/* Status badge */}
                  <span className={`shrink-0 text-[10px] font-bold px-2 py-0.5 rounded-full ${post.status ? "bg-slate-100 text-slate-950" : "bg-slate-50 text-slate-500"}`}>
                    {post.status ? "Live" : "Draft"}
                  </span>
                </div>
                {post.shortDesc && (
                  <p className="text-sm text-slate-500 mb-4 line-clamp-2 leading-relaxed">{post.shortDesc}</p>
                )}
                ...
                {post.tags && post.tags.length > 0 && (
                  <div className="flex flex-wrap gap-1 mb-3">
                    {post.tags.slice(0, 3).map((tag) => (
                      <span key={tag.id} className="text-[10px] px-2 py-0.5 rounded-full bg-slate-50 text-slate-500">
                        #{tag.title}
                      </span>
                    ))}
                  </div>
                ...
                <div className="flex items-center justify-between pt-3 border-t border-slate-200">
                  <div className="flex items-center gap-2.5">
                    <div className="h-8 w-8 rounded-full bg-sky-500 text-slate-950 flex items-center justify-center">
                      <span className="text-slate-950 text-xs font-semibold">
                        {(post.author?.name ?? "?").split(" ").map((w) => w[0]).join("").slice(0, 2).toUpperCase()}
                      </span>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-slate-950">{post.author?.name ?? "Unknown"}</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-white/60 backdrop-blur-sm">
          <div className="w-full max-w-sm rounded-2xl bg-white border border-slate-200 p-6 shadow-2xl space-y-4">
            ...
            <div className="flex gap-3">
              <button onClick={() => setDeleteConfirm(null)} className="flex-1 py-2.5 rounded-xl border border-slate-200 text-slate-700 hover:bg-slate-50 transition-colors">
                Cancel
              </button>
              <button
                onClick={() => handleDelete(deleteConfirm)}
                className="flex-1 py-2.5 rounded-xl bg-sky-500 text-slate-950 hover:bg-sky-400 transition-colors"
              >
                Delete
              </button>
            </div>

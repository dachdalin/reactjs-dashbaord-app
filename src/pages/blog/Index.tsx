...
function PostModal({ post, authorId, onClose, onSaved }: PostModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl bg-black border border-white/10 shadow-2xl">
        ...
        <form onSubmit={handleSave} className="p-6 space-y-5">
          {error && (
            <div className="p-3 rounded-xl bg-white/10 border border-white/20 text-white text-sm">{error}</div>
          )}
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white placeholder-white/50 focus:outline-none focus:ring-2 focus:ring-white/50 focus:border-transparent"
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white placeholder-white/50 focus:outline-none focus:ring-2 focus:ring-white/50 focus:border-transparent resize-none"
            ...
            <div className="flex items-end pb-1">
              <label className="flex items-center gap-3 cursor-pointer">
                <div
                  ...
                  className={`w-12 h-6 rounded-full relative transition-colors ${status ? "bg-white text-black" : "bg-white/20"} cursor-pointer`}
                >
          ...
          <div>
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white text-sm placeholder-white/50 focus:outline-none focus:ring-1 focus:ring-white/50"
                placeholder="Create new tag..."
              />
              <button type="button" onClick={handleCreateTag} className="px-3 py-2 rounded-lg bg-white/10 text-white/80 hover:bg-white/20 text-sm transition-colors">
                Add
              </button>
            </div>
          </div>
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button
              ...
              className="px-5 py-2.5 rounded-xl bg-white text-black font-medium hover:opacity-90 transition-opacity disabled:opacity-50"
            >
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
        ...
        {isAuthor() && (
          <button
            ...
            className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-white text-black text-sm font-medium shadow-lg shadow-black/30 hover:shadow-black/40 transition-all hover:scale-105"
          >
      ...
      {actionError && (
        <div className="p-3 rounded-xl bg-white/10 border border-white/20 text-white text-sm flex items-center gap-2">
          ...
          {actionError}
          <button onClick={() => setActionError(null)} className="ml-auto text-white/60 hover:text-white">✕</button>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map((post) => (
            <article
              key={post.id}
              className="group rounded-2xl overflow-hidden bg-white/5 backdrop-blur-xl border border-white/10 hover:border-white/20 transition-all hover:-translate-y-1 hover:shadow-2xl hover:shadow-black/20"
            >
              {/* Image / Type banner */}
              {post.image ? (
                <div className="relative h-48 overflow-hidden">
                  ...
                  <span className="absolute bottom-3 left-3 px-3 py-1 rounded-full text-xs font-semibold bg-white text-black">
                    {post.type}
                  </span>
                </div>
              ) : (
                <div className="h-14 bg-white text-black flex items-center px-4">
                  <span className="text-black font-semibold text-sm tracking-wide">{post.type}</span>
                </div>
              )}
              ...
              <div className="p-5">
                <div className="flex items-start justify-between gap-2 mb-2">
                  <h3 className="text-lg font-semibold text-white line-clamp-2 group-hover:text-white transition-colors flex-1">
                    {post.title}
                  </h3>
                  {/* Status badge */}
                  <span className={`shrink-0 text-[10px] font-bold px-2 py-0.5 rounded-full ${post.status ? "bg-white/20 text-white" : "bg-white/10 text-white/60"}`}>
                    {post.status ? "Live" : "Draft"}
                  </span>
                </div>
                {post.shortDesc && (
                  <p className="text-sm text-white/60 mb-4 line-clamp-2 leading-relaxed">{post.shortDesc}</p>
                )}
                ...
                {post.tags && post.tags.length > 0 && (
                  <div className="flex flex-wrap gap-1 mb-3">
                    {post.tags.slice(0, 3).map((tag) => (
                      <span key={tag.id} className="text-[10px] px-2 py-0.5 rounded-full bg-white/10 text-white/60">
                        #{tag.title}
                      </span>
                    ))}
                  </div>
                ...
                <div className="flex items-center justify-between pt-3 border-t border-white/10">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-white/40">{new Date(post.createdAt).toLocaleDateString()}</p>
                    </div>
                  </div>
        ...
        <div className="flex flex-col items-center justify-center py-16 rounded-2xl bg-white/5 border border-white/10">
          ...
          <p className="text-white/60 text-lg font-medium">No posts found</p>
          ...
          <div className="w-full max-w-sm rounded-2xl bg-black border border-white/10 p-6 shadow-2xl space-y-4">
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 py-2.5 rounded-xl bg-white text-black hover:bg-white text-black transition-colors"
              >
                Delete
              </button>
            </div>

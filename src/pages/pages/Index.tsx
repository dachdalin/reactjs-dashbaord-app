...
const PAGE_META: Record<PageType, { label: string; desc: string; color: string; bar: string; icon: React.ReactNode }> = {
  ABOUT: {
    label: "About",
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {PAGE_TYPES.map((type) => {
            ...
            return (
              <div
                key={type}
                className="group relative rounded-2xl bg-white border border-slate-200 hover:border-slate-300 hover:-translate-y-1 hover:shadow-xl transition-all overflow-hidden cursor-pointer"
              ...
              >
                ...
                <div className="p-6">
                  <div className="flex items-start justify-between mb-4">
                    <div className={`h-12 w-12 rounded-xl flex items-center justify-center shadow-xs ${meta.color}`}>
                      {meta.icon}
                    </div>
                    {/* Status */}
                    {loading ? (

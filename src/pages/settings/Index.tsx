    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="rounded-2xl bg-white/5 backdrop-blur-xl border border-white/10 p-6">
            ...
            <form onSubmit={saveProfile} className="space-y-4">
              {profileMsg && (
                <div className={`p-3 rounded-xl text-sm ${profileMsg.type === "success" ? "bg-emerald-500/10 border border-emerald-500/20 text-emerald-400" : "bg-red-500/10 border border-red-500/20 text-red-400"}`}>
                  {profileMsg.text}
                </div>
              )}
              <div>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-2">Position / Job Title</label>
              ...
              <div className="flex justify-end">
                <button type="submit" disabled={profileSaving}
                  className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-medium hover:opacity-90 disabled:opacity-50 transition-opacity">
          ...
          <div className="rounded-2xl bg-white/5 backdrop-blur-xl border border-white/10 p-6">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between p-4 bg-white/5 rounded-xl">
                ...
                <button className="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors">Enable</button>
              </div>
              <div className="flex items-center justify-between p-4 bg-white/5 rounded-xl">
                <div>
                ...
                <div className="space-y-3">
                      ...
                      {editingId === s.id ? (
                        <>
                          <input value={editKey} onChange={(e) => setEditKey(e.target.value)}
                            className="flex-1 px-3 py-2 rounded-lg bg-white/10 border border-white/20 text-white text-sm focus:outline-none focus:ring-1 focus:ring-purple-500"
                            placeholder="Key" />
                          <input value={editValue} onChange={(e) => setEditValue(e.target.value)}
                            className="flex-1 px-3 py-2 rounded-lg bg-white/10 border border-white/20 text-white text-sm focus:outline-none focus:ring-1 focus:ring-purple-500"
                            placeholder="Value" />
                          <button onClick={() => handleUpdateSetting(s.id)} className="px-3 py-2 rounded-lg bg-indigo-600 text-white text-sm hover:bg-indigo-700 transition-colors">Save</button>
                          <button onClick={() => setEditingId(null)} className="px-3 py-2 rounded-lg bg-white/10 text-gray-300 text-sm hover:bg-white/20 transition-colors">Cancel</button>
                        </>
                      ) : (
                        <>
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium text-white font-mono truncate">{s.key}</p>
                            <p className="text-xs text-gray-400 truncate font-mono">{s.value}</p>
                          </div>
                  ...
                  <div className="flex items-center gap-3 p-4 bg-white/5 rounded-xl border border-dashed border-white/20">
                    <input value={newKey} onChange={(e) => setNewKey(e.target.value)}
                      className="flex-1 px-3 py-2 rounded-lg bg-white/10 border border-white/20 text-white text-sm placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-purple-500"
                      placeholder="New key..." />
                    <input value={newValue} onChange={(e) => setNewValue(e.target.value)}
                      onKeyDown={(e) => e.key === "Enter" && handleCreateSetting()}
                      className="flex-1 px-3 py-2 rounded-lg bg-white/10 border border-white/20 text-white text-sm placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-purple-500"
                      placeholder="Value..." />
                    <button onClick={handleCreateSetting} className="px-3 py-2 rounded-lg bg-indigo-600 text-white text-sm hover:bg-indigo-700 transition-colors">
                      Add
                    </button>
                  </div>
                </div>
        ...
        <div className="space-y-6">
          {/* Account Status */}
          <div className="rounded-2xl bg-gradient-to-br from-indigo-600/50 to-purple-600/50 backdrop-blur-xl border border-white/10 p-6">
            ...
            {user && (
              <div className="space-y-1 text-sm">
                ...
                {user.emailVerified !== undefined && (
                  <p className="text-gray-300">
                    Email: <span className={user.emailVerified ? "text-emerald-400" : "text-yellow-400"}>
            ...
            <ul className="space-y-2 text-sm">
              {[
                ...
                <li key={cap.label} className="flex items-center gap-2">
                  <div className={`h-4 w-4 rounded-full flex items-center justify-center shrink-0 ${cap.allowed ? "bg-emerald-500/20 text-emerald-400" : "bg-gray-500/20 text-gray-500"}`}>
                    {cap.allowed ? "✓" : "✕"}
                  </div>
